				       ...
				       unsigned long attrs)
{
	...
	ret = vmsg_ffa_bus_area_share(to_ffa_dev(dev), vaddr, n_pages, dma_handle);
	if (ret) {
					       ...
					       unsigned long attrs)
{
	...
	swiotlb_dma = swiotlb_map(dev, page_to_phys(page) + offset, size, dir, attrs);
	if (swiotlb_dma == DMA_MAPPING_ERROR)
		return DMA_MAPPING_ERROR;

	dma_offset = offset_in_page(swiotlb_dma);
	dma_handle = swiotlb_dma - dma_offset;
	if (vmsg_ffa_bus_area_share(to_ffa_dev(dev), phys_to_virt(dma_handle),
				    n_pages, &dma_handle)) {

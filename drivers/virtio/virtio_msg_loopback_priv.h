...
/*
 * Virtio message loopback base scaffold private interfaces.
 *
 * Copyright (c) 2026 Arm Limited or its affiliates. All rights reserved.
 */

#ifndef _VIRTIO_MSG_LOOPBACK_PRIV_H
#define _VIRTIO_MSG_LOOPBACK_PRIV_H
...
enum virtio_msg_loopback_map_state {
	...
	VIRTIO_MSG_LOOPBACK_MAP_STATE_REMOTE_RELEASED = 4,
};
...
struct virtio_msg_loopback_page_window {
	...
	bool revoke_settled;
	/*
	 * The bridge reports MAP_ADD completion to the loopback provider through
	 * split response/release helpers. Rejected map adds invalidate the
	 * bridge session and fail the pending add completion.
	 */
	struct completion map_add_done; /* signalled after MAP_ACK_OK */
	int map_add_status;             /* 0 for accepted map adds */
	bool map_add_settled;           /* true once accepted add is reported */
	refcount_t refs;
	void *kva;
...
struct virtio_msg_loopback_slot {
	...
	struct virtio_msg_bus_queue_helper ordered_queue;
	atomic_t stopping;
	bool dma_initial_pool_installing;
	bool dma_initial_pool_seen;
	int dma_initial_pool_status;
...
struct virtio_msg_loopback {
	...
	/* Protects pending_relay_xa and relay/token sequence allocation. */
	struct mutex pending_lock;
	struct xarray pending_relay_xa;
	u64 next_relay_seq;
	u16 next_token;
	/* Protects exact-export/page-window state and bus-address lookup. */
	struct mutex area_lock;
	struct xarray map_area_xa_by_dma_addr;
	struct virtio_msg_bus_dma_device_helper map_helper;
	spinlock_t exact_lock; /* Protects exact_exports list and atomic transitions. */
	struct list_head exact_exports;
	struct workqueue_struct *exact_cleanup_wq;
	u64 map_next_offset;
};
...
#endif /* _VIRTIO_MSG_LOOPBACK_PRIV_H */

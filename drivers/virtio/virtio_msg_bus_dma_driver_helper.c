...
/*
 * Pool chunk size in KiB.  Each pre-registered chunk is exported once to the
 ...
 */
static unsigned long virtio_msg_dma_pool_chunk_size_kb = 512;
module_param(virtio_msg_dma_pool_chunk_size_kb, ulong, 0444);
MODULE_PARM_DESC(virtio_msg_dma_pool_chunk_size_kb,
		 "DMA pool chunk size in KiB (0 = disabled, default: 512)");
...
enum virtio_msg_bus_dma_pool_remote_state {
	VIRTIO_MSG_BUS_DMA_POOL_REMOTE_PUBLISHED = 0,
	VIRTIO_MSG_BUS_DMA_POOL_REMOTE_RELEASED,
	VIRTIO_MSG_BUS_DMA_POOL_REMOTE_QUARANTINED,
};
...
struct virtio_msg_bus_dma_mapping {
	...
	struct virtio_msg_bus_dma_driver_export backing;
...
struct virtio_msg_bus_dma {
	...
	void *provider_ctx;
	struct virtio_msg_bus_dma_driver_provider_caps provider_caps;
...
struct virtio_msg_bus_dma_pool_chunk {
	...
	spinlock_t lock;               /* protects bitmap and remote state */
	...
	unsigned int active_suballocs;  /* active dependent DMA mappings */
	enum virtio_msg_bus_dma_pool_remote_state remote_state;
...
static LIST_HEAD(virtio_msg_bus_dma_orphaned_pool_chunks);
static DEFINE_SPINLOCK(virtio_msg_bus_dma_orphaned_pool_chunks_lock);
...
bool
virtio_msg_bus_dma_driver_helper_is_installed(const struct virtio_msg_transport_device *vmdev)
{
	return vmdev && vmdev->dma_shim && vmdev->dma_shim->shim_installed;
}
EXPORT_SYMBOL_GPL(virtio_msg_bus_dma_driver_helper_is_installed);

struct device *
virtio_msg_bus_dma_driver_helper_delegate_dev(const struct virtio_msg_transport_device *vmdev)
{
	if (!virtio_msg_bus_dma_driver_helper_is_installed(vmdev))
		return NULL;

	return vmdev->dma_shim->delegate_dev;
}
EXPORT_SYMBOL_GPL(virtio_msg_bus_dma_driver_helper_delegate_dev);

int
virtio_msg_bus_dma_export_released(const struct virtio_msg_bus_dma_driver_export *export,
				   struct virtio_msg_bus_dma_driver_release *release)
{
	...
	if (!export)
		return -EINVAL;
	if (!(export->bridge_flags &
	      VIRTIO_MSG_BUS_BRIDGE_MAP_F_RETENTION_REQUESTED))
		return 0;
	if (export->type != VIRTIO_MSG_BUS_DMA_DRIVER_EXPORT_POOL_CHUNK)
		return -EOPNOTSUPP;
	if (!export->release_ctx)
		return -EINVAL;

	chunk = export->release_ctx;

	spin_lock_irqsave(&chunk->lock, flags);
	if (chunk->active_suballocs) {
		chunk->remote_state =
			VIRTIO_MSG_BUS_DMA_POOL_REMOTE_QUARANTINED;
		if (release) {
			release->action =
				VIRTIO_MSG_BUS_DMA_DRIVER_RELEASE_RETAINED_ACTIVE;
			release->active_suballocs = chunk->active_suballocs;
		}
	} else {
		chunk->remote_state =
			VIRTIO_MSG_BUS_DMA_POOL_REMOTE_RELEASED;
	...
	if (release)
		release->release_ctx = export->release_ctx;

	return 0;
}
EXPORT_SYMBOL_GPL(virtio_msg_bus_dma_export_released);
...
static bool virtio_msg_bus_dma_provider_revoke_in_progress(int ret);
static void
virtio_msg_bus_dma_pool_chunk_orphan
		(struct virtio_msg_bus_dma_pool_chunk *chunk);
				  ...
				  unsigned int *out_page_offset)
{
	...
	list_for_each_entry(chunk, &dma->pool_chunks, node) {
		...
		if (chunk->remote_state !=
		    VIRTIO_MSG_BUS_DMA_POOL_REMOTE_PUBLISHED)
			goto next_chunk;
				 ...
				 unsigned int page_offset, unsigned int npages)
{
	...
	chunk->free_pages += npages;
	if (WARN_ON_ONCE(!chunk->active_suballocs))
		chunk->remote_state = VIRTIO_MSG_BUS_DMA_POOL_REMOTE_QUARANTINED;
	else
		chunk->active_suballocs--;
	spin_unlock_irqrestore(&chunk->lock, flags);
}
...
virtio_msg_bus_dma_pool_chunk_alloc(struct virtio_msg_bus_dma *dma, gfp_t gfp)
{
	...
	chunk->remote_state = VIRTIO_MSG_BUS_DMA_POOL_REMOTE_PUBLISHED;
	...
	chunk->export.type = VIRTIO_MSG_BUS_DMA_DRIVER_EXPORT_POOL_CHUNK;
	chunk->export.bridge_flags =
		VIRTIO_MSG_BUS_BRIDGE_MAP_F_RETENTION_REQUESTED;
	chunk->export.release_ctx = chunk;

	ret = dma->provider_ops->export_add(dma->provider_ctx, &chunk->export,
					    &chunk->provider_handle, gfp);
	if (ret) {
		dev_dbg(dma->delegate_dev,
			"dma-pool: provider export failed dma=%pad len=%zu pages=%u ret=%d\n",
		       &chunk->export.local_dma_addr, chunk->export.length,
		       chunk->export.npages, ret);
		goto err_free_pages;
	}
...
err_revoke_chunk_export:
	revoke_ret = dma->provider_ops->export_del(dma->provider_ctx,
						   &chunk->export,
						   chunk->provider_handle,
						   false);
	if (virtio_msg_bus_dma_provider_revoke_in_progress(revoke_ret)) {
		virtio_msg_bus_dma_pool_chunk_orphan(chunk);
		return ERR_PTR(revoke_ret);
	}
	if (revoke_ret && revoke_ret != -EOPNOTSUPP)
		dev_warn_ratelimited(dma->delegate_dev,
				     "dma-pool: rollback revoke failed dma=%pad len=%zu ret=%d\n",
			   &chunk->export.dma_addr, chunk->length, revoke_ret);
...
static bool virtio_msg_bus_dma_provider_revoke_in_progress(int ret)
{
	return ret == VIRTIO_MSG_BUS_DMA_PROVIDER_REVOKE_IN_PROGRESS ||
	       ret == -EAGAIN;
}
		...
		(struct virtio_msg_bus_dma_pool_chunk *chunk)
{
	...
	spin_lock_irqsave(&virtio_msg_bus_dma_orphaned_pool_chunks_lock, flags);
	list_add_tail(&chunk->node, &virtio_msg_bus_dma_orphaned_pool_chunks);
	spin_unlock_irqrestore(&virtio_msg_bus_dma_orphaned_pool_chunks_lock,
			       flags);
}
				   ...
				   struct virtio_msg_bus_dma_pool_chunk *chunk)
{
	...
	if (chunk->provider_handle) {
		ret = dma->provider_ops->export_del(dma->provider_ctx,
						    &chunk->export,
						    chunk->provider_handle,
						    true);
		if (virtio_msg_bus_dma_provider_revoke_in_progress(ret))
			return ret;
		if (ret && ret != -EOPNOTSUPP)
			dev_warn_ratelimited(dma->delegate_dev,
					     "dma-pool: provider revoke failed dma=%pad len=%zu ret=%d\n",
				   &chunk->export.dma_addr, chunk->length,
				   ret);
		chunk->provider_handle = NULL;
	}
...
static void virtio_msg_bus_dma_pool_grow_work(struct work_struct *work)
{
	...
	if (IS_ERR(new_chunk)) {
		dev_dbg(dma->delegate_dev,
			"dma-pool: deferred grow failed chunk_size=%zu min_pages=%u ret=%ld\n",
		       chunk_size, min_pages, PTR_ERR(new_chunk));
		return;
	}
...
virtio_msg_bus_dma_pool_init(struct virtio_msg_bus_dma *dma)
{
	...
	chunk_size = (size_t)virtio_msg_dma_pool_chunk_size_kb << 10;
	if (!chunk_size)
		return 0;
...
virtio_msg_bus_dma_pool_cleanup(struct virtio_msg_bus_dma *dma)
{
	...
	list_for_each_entry_safe(chunk, tmp, &to_free, node) {
		...
		ret = virtio_msg_bus_dma_pool_chunk_free(dma, chunk);
		if (virtio_msg_bus_dma_provider_revoke_in_progress(ret)) {
			dev_warn_ratelimited(dma->delegate_dev,
					     "dma-pool: retaining chunk dma=%pad len=%zu with revoke in progress\n",
				   &chunk->export.dma_addr, chunk->length);
			virtio_msg_bus_dma_pool_chunk_orphan(chunk);
		}
	}
...
static void
virtio_msg_bus_dma_driver_cleanup_drain_local_frees
	(struct virtio_msg_bus_dma *dma,
	 struct list_head *pending)
{
	struct virtio_msg_bus_dma_mapping *mapping;
					    ...
					    gfp_t gfp)
{
	...
	mapping->backing.type = VIRTIO_MSG_BUS_DMA_DRIVER_EXPORT_POOL_CHUNK;
	mapping->backing.bridge_flags =
		VIRTIO_MSG_BUS_BRIDGE_MAP_F_RETENTION_REQUESTED;
	mapping->backing.release_ctx = chunk;
...
virtio_msg_bus_dma_validate_provider_export
	(const struct virtio_msg_bus_dma_mapping *mapping,
	 const struct virtio_msg_bus_dma_driver_export *expected_export)
{
	...
	if (provider_export->local_dma_addr != expected_export->local_dma_addr ||
	    ...
	    provider_export->type != expected_export->type ||
	    provider_export->bridge_flags != expected_export->bridge_flags ||
	    provider_export->release_ctx != expected_export->release_ctx)
		return -EOPNOTSUPP;
...
{
	struct virtio_msg_bus_dma_driver_export expected_export;
	int ret;
	 ...
	 enum virtio_msg_bus_dma_mapping_kind kind)
{
	...
	mapping->backing.type = VIRTIO_MSG_BUS_DMA_DRIVER_EXPORT_ONESHOT;
	mapping->backing.bridge_flags = 0;
	mapping->backing.release_ctx = mapping;
	 ...
	 size_t length)
{
	...
	mapping = xa_load(&dma->active_exports, index);
	 ...
	 struct virtio_msg_bus_dma_mapping *mapping)
{
	...
	index = (unsigned long)mapping->key.dma_addr;
	xa_lock_irqsave(&dma->active_exports, flags);
	spin_lock(&dma->active_exports_lock);
	ret = virtio_msg_bus_dma_xa_insert_unique_locked
		(&dma->active_exports, index, mapping, GFP_ATOMIC);
	...
	(struct virtio_msg_bus_dma *dma, dma_addr_t dma_addr, size_t length)
{
	...
	if (mapping &&
	    !virtio_msg_bus_dma_xa_erase_if_match_locked
		    (&dma->active_exports, index, mapping))
		mapping = NULL;
	spin_unlock(&dma->active_exports_lock);
	xa_unlock_irqrestore(&dma->active_exports, flags);
...
virtio_msg_bus_dma_delegate_from_map(union virtio_map map)
{
	...
	return virtio_msg_bus_dma_driver_helper_delegate_dev(vmdev);
}
...
static void
virtio_msg_bus_dma_driver_provider_caps_init(struct virtio_msg_bus_dma *dma)
{
...
int
virtio_msg_bus_dma_driver_helper_install(struct virtio_msg_transport_device
					 *vmdev,
					 ...
					 void *provider_ctx)
{
	...
	virtio_msg_bus_dma_driver_provider_caps_init(dma);
...
EXPORT_SYMBOL_GPL(virtio_msg_bus_dma_driver_helper_install);

void
virtio_msg_bus_dma_driver_helper_cleanup(struct virtio_msg_transport_device
					 *vmdev)
{
	struct virtio_msg_bus_dma_mapping *mapping;
	struct virtio_msg_bus_dma *dma;
	...
	virtio_msg_bus_dma_driver_cleanup_drain_local_frees(dma,
							    &deferred_local);
...
EXPORT_SYMBOL_GPL(virtio_msg_bus_dma_driver_helper_cleanup);

...
/*
 * Virtio message transport public API.
 *
 ...
 *
 * This header exposes the provider-facing transport device contract without
 * exposing private request, config, status, virtqueue, or event runtime state.
 */
 ...
 * callback handles request/response exchanges in sleepable context under
 * transport request serialization. Notify events may still enter providers
 * directly from non-sleepable transport notify paths. Providers own bus-side
 * queueing, retry/drop policy, and request token/correlation state.
